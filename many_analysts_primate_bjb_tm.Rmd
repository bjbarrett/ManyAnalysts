...
m0exp <- ulam(
    ...
    ), data=datalist , chains=4 , cores=8 , iter=2000 , cmdstan=TRUE )
...
```
So $\alpha$ implies a median grooming rate of `r median(exp(post$a)` minutes/hour. We also see more varaiation in dyads then we do in individuals, and more variation in receiving grooming(`sigma_gr[2]`) thatn giving grooming (`sigma_gr[1]`).
...
m0expZI <- ulam(
    alist(
        ...
        logit(pAB) <- a_z + gr[A_id,1] + gr[B_id,2] + log(exposure_dyad_hour),
        logit(pBA) <- a_z + gr[B_id,1] + gr[A_id,2] + log(exposure_dyad_hour),
...
#         groomAB ~ dzipois( pAB, lambda_AB ),
#         groomBA ~ dzipois( pBA , lambda_BA ),
#         logit(pAB) <- a_z + gr[A_id,1] + gr[B_id,2] + d_z[dyad_id,1] + log(exposure_dyad_hour),
#         logit(pBA) <- a_z + gr[B_id,1] + gr[A_id,2] + d_b[dyad_id,2] + log(exposure_dyad_hour),
#         log(lambda_AB) <- a + gr[A_id,3] + gr[B_id,4] + d[dyad_id,1]  + log(exposure_dyad_hour),
#         log(lambda_BA) <- a + gr[B_id,3] + gr[A_id,4] + d[dyad_id,2]  + log(exposure_dyad_hour),
#         a ~ normal(1,2),
#         a_z ~ normal(0,1),
...
precis(m0expZI , pars=c("a", "a_z" , "sigma_d" ,"sigma_dz" , "sigma_gr" , "Rho_d" , "Rho_gr") , depth=3)
...
m0expZI1 <- ulam(
    alist(
        ...
        logit(pAB) <- a_z + gr[A_id,1] + gr[B_id,2]  +  log(exposure_dyad_hour),
        logit(pBA) <- a_z + gr[B_id,1] + gr[A_id,2]  + log(exposure_dyad_hour),
        log(lambda_AB) <- a + gr[A_id,3] + gr[B_id,4]  + log(exposure_dyad_hour),
        log(lambda_BA) <- a + gr[B_id,3] + gr[A_id,4]  + log(exposure_dyad_hour),
        a ~ normal(1,2),
        a_z ~ normal(0,1)
      vector[4]:gr[N_i] ~ multi_normal(0,Rho_gr,sigma_gr),
      Rho_gr ~ lkj_corr(4),
      sigma_gr ~ exponential(1)
       ...
       # transpars> matrix[N_dyads,4]:d <-
        #        compose_noncentered( sigma_d , L_Rho_d , zd ),
...
m0expZI2 <- ulam(
    alist(
        groomAB ~ dzipois( pAB, lambda_AB ),
        groomBA ~ dzipois( pBA , lambda_BA ),
        logit(pAB) <- a_z + gr_z[A_id,1] + gr_z[B_id,2] + d_z[dyad_id,1]  + log(exposure_dyad_hour),
        logit(pBA) <- a_z + gr_z[B_id,1] + gr_z[A_id,2] + d_z[dyad_id,2]  + log(exposure_dyad_hour),
        log(lambda_AB) <- a + gr[A_id,1] + gr[B_id,2] + d[dyad_id,1]  + log(exposure_dyad_hour),
        log(lambda_BA) <- a + gr[B_id,1] + gr[A_id,2] + d[dyad_id,2]  + log(exposure_dyad_hour),
...
precis(m0expZI2 , pars=c("Rho_d") , depth=3)
summary(m0expZI2  )
...
m0expZI4 <- ulam(
    alist(
        ...
        sigma_gr ~ exponential(1),
       ## dyad effects choleskyfied
      combo_sigma <- to_vector( sigma_dB , sigma_dB , sigma_dG , sigma_dG) ,
      vector[4]:d[N_i] ~ multi_normal(0,Rho_d, combo_sigma),
      Rho_d ~ lkj_corr(4),
      sigma_dB ~ exponential(1),
      sigma_dG ~ exponential(1)
      #  transpars> matrix[N_dyads,4]:d <-
      #        compose_noncentered( rep_row_vector(sigma_d,4), L_Rho_d , zd ),
      #  matrix[4,N_dyads]:zd ~ normal( 0 , 1 ),
      #  cholesky_factor_corr[4]:L_Rho_d ~ lkj_corr_cholesky( 4 ),
      #  sigma_d ~ exponential(1),
      #  ## compute correlation matrix for dyads
...
post <- extract.samples(m0expZI4)
precis(m0expZI4 , pars=c("Rho_d" , "sigma") , depth=3)
```
